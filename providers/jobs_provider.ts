...
export default class JobsProvider {
  ...
  async boot() {
    // Auto-discovery is handled by the job:listen command
    // No need to discover jobs during application boot since
    // jobs are always processed via `node ace job:listen`
  }
